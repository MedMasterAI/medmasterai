...
export const createJob = functions.https.onRequest(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      console.log(
        `createJob end - duration: ${Date.now() - startTime}ms rss: ${process.memoryUsage().rss}`
      );
    }
  }
);
...
async function processJob(jobId: string) {
  ...
  try {
    ...
    try { await GLOBAL_JOBS_REF.update({ count: FieldValue.increment(-1) }); } catch (e) { console.error('decrement globalJobs', e); }
  }
}
...
export const worker = functions.pubsub.onMessagePublished(
  ...
  async event => {
    ...
    try {
      ...
      console.log(
        `worker end - duration: ${Date.now() - startTime}ms rss: ${process.memoryUsage().rss}`
      );
    }
  }
);
...
export const getJobStatus = functions.https.onRequest(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!jobId) {
      ...
      console.log(
        `getJobStatus end - duration: ${Date.now() - startTime}ms rss: ${process.memoryUsage().rss}`
      );
      return;
    }
...
export const downloadJobResult = functions.https.onRequest(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!jobId) {
      res.status(400).json({ error: 'jobId required' });
      console.log(
        `downloadJobResult end - duration: ${Date.now() - startTime}ms rss: ${process.memoryUsage().rss}`
      );
      return;
    }
    const doc = await db.collection('jobs').doc(jobId).get();
...
export const cleanupOldJobs = onSchedule('every 24 hours', async () => {
  ...
  for (const doc of snap.docs) {
    const data = doc.data() as any;
    if (data.resultPath) {
      try { await storage.file(data.resultPath).delete(); } catch (err) { console.error('cleanupOldJobs delete', err); }
    }
    await doc.ref.delete();
  }
  console.log(
    `cleanupOldJobs end - duration: ${Date.now() - startTime}ms rss: ${process.memoryUsage().rss}`
  );
});
