...
export default function LUPPage() {
  ...
  const [materiaPrioridad, setMateriaPrioridad] = useState<
    "alta" | "media" | "baja"
  >("media");
  ...
  const agregarMateria = () => {
    ...
    setMaterias([
      ...materias,
      {
        ...
        nombre: n,
        prioridad: materiaPrioridad,
        temas: [],
      },
    ]);
    setMateriaInput("");
    setMateriaPrioridad("media");
  };
  ...
  const updateMateria = (id: string, field: keyof Materia, value: any) => {
    setMaterias(
      materias.map((m) => (m.id === id ? { ...m, [field]: value } : m)),
    );
  };
    ...
    try {
      ...
      if (user) {
        ...
        const ref = await addDoc(
          collection(db, "users", user.uid, "lupPlans"),
          {
    ...
    <div className="max-w-xl mx-auto p-4 space-y-6">
      {step === 0 && (
        <div className="space-y-4 text-center">
          ...
          <p>
            Organiza tu estudio, no tu estrés. Comienza agregando tus materias.
          </p>
      ...
      {step === 1 && (
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            />
            <select
              value={materiaPrioridad}
              onChange={(e: ChangeEvent<HTMLSelectElement>) =>
                setMateriaPrioridad(e.target.value as "alta" | "media" | "baja")
              }
            ...
            {materias.map((m) => (
              <li key={m.id} className="flex gap-2 items-end">
                <Input
                  value={m.nombre}
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    updateMateria(m.id, "nombre", e.target.value)
                  }
                  className="flex-1"
                />
                <select
          ...
          <Button onClick={() => setStep(2)} disabled={materias.length === 0}>
            Siguiente
          </Button>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          {materias.map((m) => (
            <div key={m.id} className="space-y-2">
              ...
              />
              <Button
                onClick={() => agregarTemas(m.id, temasPorMateria[m.id] || "")}
              >
                Agregar temas
              </Button>
              {m.temas.length > 0 && (
                  ...
                  {m.temas.map((t, i) => (
                    <li key={i} className="flex gap-2 items-end">
                      <Input
                        value={t.nombre}
                        onChange={(e: ChangeEvent<HTMLInputElement>) =>
                          editTema(m.id, i, "nombre", e.target.value)
                        }
                        className="flex-1"
                      />
                      <select
                        ...
                        onChange={(e: ChangeEvent<HTMLSelectElement>) =>
                          editTema(
                            ...
                            i,
                            "dificultad",
                            e.target.value as Tema["dificultad"],
                          )
                        }
                        className="border border-border bg-background text-foreground rounded-md px-2 py-2 text-sm"
                      >
        ...
        <div className="space-y-4">
          ...
          {plan.length === 0 ? (
            <div className="space-y-2">
              <p>
                No hay bloques suficientes para cubrir todos los temas antes de
                las fechas.
              </p>
            ...
            <table className="w-full text-sm border">
              ...
              {(() => {
                ...
                const start = sorted.length
                  ? new Date(sorted[0].fecha)
                  : new Date();
                const weeks: Record<number, typeof plan> = {};
                sorted.forEach((item) => {
                  const diff = Math.floor(
                    (new Date(item.fecha).getTime() - start.getTime()) /
                      86400000,
                  );
                  const w = Math.floor(diff / 7);
                  weeks[w] = [...(weeks[w] || []), item];
                });
                return (
                  <tbody>
